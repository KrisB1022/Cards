...
it("has 1 <ModalFooter/>, reset and close <Button/>", () => {
  ...
  expect(resetBtn.prop("children")).toMatch("Reset");

  const closeBtn = wrapper.find(Button).at(2);
  expect(closeBtn.exists()).toBeTruthy();
  expect(closeBtn.length).toBe(1);
  expect(closeBtn.prop("children")).toMatch("Close");
});
